...
// before Talon was booted, or because we have denied all tokens with the same JTI
...
func (p *WebsocketPayload) Denylisted() bool {
	...
	// If the time that the token was issued is before the time at which Talon was booted
	// then the token is invalid for our purposes, even if the token "has permission".
	if p.IssuedAt.Time.Before(talonBootTime) {
		return true
	}
